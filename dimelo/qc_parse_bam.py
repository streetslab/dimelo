...
def get_mod_reference_positions_by_mod(read, basemod, index, threshA, threshC):
...
def batch_read_generator(file_bamIn, batch_size, hasbaseq):
    counter = 0
    r_list = []
    ...
    for read in file_bamIn.fetch(until_eof=True):
        ...
        if hasbaseq:
            ...
            r = [read.query_name, read.reference_name, read.reference_start, read.reference_end, read.query_length,
                 "-" if read.is_reverse else "+", read.mapping_quality, ave_qual(read.query_qualities),
                 ave_qual(read.query_alignment_qualities)]
        [(mod, numA, probs), (mod2, numC, probs2)] = get_modified_reference_positions(read, "A+CG", 128, 128)
...
def parse_bam_read(bamIn, hasbaseq=False):
    ...
    dt = ['TEXT', 'TEXT', 'INT', 'INT', 'INT', 'TEXT', 'INT', 'INT', 'INT', 'TEXT', 'FLOAT', 'FLOAT', 'FLOAT', 'FLOAT']
    table_name = 'READS'
    create_sql_table(DB_NAME, table_name, cols, dt)

    template_command = '''INSERT INTO ''' + table_name + ''' VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?);'''
    ...
    Parallel(n_jobs=num_cores, verbose=10)(
        delayed(execute_sql_command)(
            template_command,
            DB_NAME,
            i) for i in batch_read_generator(file_bamIn, 100, hasbaseq))
