...
#                     print(line)
                ...
                if read_name!=fields[0]:
                    ...
                    if index>1:
                        ...
                        h5['mod_vector'][read_counter+old_size]=np.frombuffer(gzip.compress(mod_vector.tobytes(),compresslevel=compress_level), dtype=np.uint8)
                        h5['val_vector'][read_counter+old_size]=np.frombuffer(gzip.compress(val_vector.tobytes(),compresslevel=compress_level), dtype=np.uint8)
                        read_counter+=1
