...
def retag_bam(bam_path,force_retag = False):
    ...
    """
    bam_updated_path = bam_path.parent.parent / 'output' / (Path(bam_path.stem).stem + '.updated' + bam_path.suffix)
    bam_updated_index_path = bam_path.parent.parent / 'output' / (Path(bam_path.stem).stem + '.updated' + bam_path.suffix + '.bai')
