...
def test_scan_genome_time_course_filters_incomplete_trajectories_per_window(monkeypatch):
    ...
    first_window = result.hits.loc[result.hits["window_id"] == "chr1:0-500"].iloc[0]
    second_window = result.hits.loc[result.hits["window_id"] == "chr1:500-1000"].iloc[0]
    assert first_window["n_pairs_used"] == 2
    assert second_window["n_pairs_used"] == 1
    ...
    result = region_discovery.scan_genome(
        samples=[
            ...
            SampleSpec(
                sample_id="tp1_45",
                condition="45min",
                extract_h5="tp1_45.h5",
                metadata={"pileup_path": "tp1_45.bed.gz", "pair_id": "pair-1"},
            ),
            SampleSpec(
                sample_id="tp2_0",
                condition="0min",
            ...
            SampleSpec(
                sample_id="tp2_45",
                condition="45min",
                extract_h5="tp2_45.h5",
                metadata={"pileup_path": "tp2_45.bed.gz", "pair_id": "pair-2"},
            ),
        ],
        motifs=["A,0"],
        genome_sizes={"chr1": 500},
        window_size=500,
        step_size=500,
        contrast=ContrastSpec(
            mode="time_course",
